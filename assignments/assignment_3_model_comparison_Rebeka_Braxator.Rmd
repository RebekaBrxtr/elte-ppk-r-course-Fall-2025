...
```{r}
library(readxl)
patient_data <- read_excel("/Users/rebekabraxator/Documents/University/R_programming/assignment_3_dataset.xlsx")
View(patient_data)
...
# Check pain values 
min(patient_data$pain)
max(patient_data$pain)
mean(patient_data$pain)
sd(patient_data$pain)

# Check STAI_trait 
min(patient_data$STAI_trait)
max(patient_data$STAI_trait)
mean(patient_data$STAI_trait)
sd(patient_data$STAI_trait)

#Check pain_cat
min(patient_data$pain_cat)
max(patient_data$pain_cat)
mean(patient_data$pain_cat)
sd(patient_data$pain_cat)

# Check cortisol_serum
min(patient_data$cortisol_serum)
max(patient_data$cortisol_serum)
mean(patient_data$cortisol_serum)
sd(patient_data$cortisol_serum)

# Check cortisol_saliva
min(patient_data$cortisol_saliva)
max(patient_data$cortisol_saliva)
mean(patient_data$cortisol_saliva)
sd(patient_data$cortisol_saliva)

# Check mindfulness values 
min(patient_data$mindfulness)
max(patient_data$mindfulness)
mean(patient_data$mindfulness)
sd(patient_data$mindfulness)

# Check weight 
min(patient_data$weight)
max(patient_data$weight)
mean(patient_data$weight)
sd(patient_data$weight)

# Check age 
min(patient_data$age)
max(patient_data$age)
...
min(patient_data$IQ)
max(patient_data$IQ)
mean(patient_data$IQ)
sd(patient_data$IQ)

# Count males and females and check for inconsistencies 
library(dplyr)
patient_data |> count(sex)
...
```{r}
# Remove patient with invalid values  
clean_data <- patient_data |> filter(pain >= 0 & pain <= 10)
clean_data <- clean_data |> filter(mindfulness >= 0 & mindfulness <= 6)
# Change incorrect sex category 
clean_data <- clean_data |> mutate(sex = ifelse(sex == "woman", "female", sex))
...
confint(simple_model) #Confidence interval 
lm.beta(simple_model) # It gives the effect size
AIC(simple_model)
...
```{r}
plot(complex_model, which = 2) # Normal Q-Q
```
...
```{r}
# Averaging cortisol levels because of multicollinearity
clean_data$cortisol_mean <- rowMeans(clean_data[, c("cortisol_serum", "cortisol_saliva")])
names(clean_data)
str(clean_data)


# Remove outlier participants 
outliers <- which(cook > 4/nrow(clean_data))
updated_data <- clean_data[-outliers, ]

# Updated model with cortisol mean
updated_model <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_mean, data = updated_data)
summary(updated_model)

cook_updated <- cooks.distance(updated_model)
outliers_updated <- which(cook_updated > (4 / nrow(updated_data)))
...
```{r}
plot(updated_model, which = 4)
which(cook_updated > (4 / nrow(updated_data)))
...
```{r}
updated_simple_model <- lm(pain ~ age + sex, data = updated_data)
summary(updated_simple_model)
confint(updated_simple_model)
lm.beta(updated_simple_model)
AIC(updated_simple_model)
...
```{r}
updated_complex_model <- updated_model
summary(updated_complex_model)
confint(updated_complex_model)
lm.beta(updated_complex_model)
AIC(updated_complex_model)
...
```{r}
anova(updated_simple_model, updated_complex_model)
AIC(updated_simple_model, updated_complex_model)
lm.beta(updated_complex_model)
```
